...
export class PreawardCommitteeComponent implements OnInit {
  ...
  constructor(
    private PreAwardCommitteeService: PreAwardCommitteeService,
    private convertorService: PreAwardCommitteeConvertorService
  ) { }
  ...
  loadData(): void {
    this.PreAwardCommitteeService.getPreAwardCommitteeData().subscribe(
  ...
  exportPDF(): void {
    ...
    const docDefinition = {
      ...
      content: [
        { text: 'Pre-Award Committee Report', style: 'header' },
    ...
    pdfMake.createPdf(docDefinition).download('PreAwardCommitteeReport.pdf');
  }
  ...
  private calculateAverageScore(company: Company): string {
  ...
  private isLowestPrice(price: number, prices: number[]): boolean {
