...
export class BidComparisonTableComponent implements OnInit {
  ...
  private async addProductQuestions(worksheet: ExcelJS.Worksheet, headerStyle: Partial<ExcelJS.Style>, borderStyle: Partial<ExcelJS.Borders>) {
    ...
    this.bidData.productQuestions.forEach(category => {
      ...
      let priceQuestion: Question | null = null;
      const pricingSection = category.sections.find(s => s.name === 'Pricing');
      if (pricingSection) {
        priceQuestion = pricingSection.questions.find(q =>
          q.question === 'Total price for this product/service' &&
          q.answers.some(a => a !== null && a !== '')
        );
      }

      if (priceQuestion) {
        const priceRow = worksheet.addRow(['Pricing', priceQuestion.question]);

        // Convert answers to integers and find the lowest value
        const priceAnswers = priceQuestion.answers.map(price =>
          typeof price === 'number' ? Math.floor(price) :
            (typeof price === 'string' && price !== '' ? Math.floor(Number(price)) : null)
        );
        const validPrices = priceAnswers.filter(price => price !== null && !isNaN(price));
        const lowestPrice = validPrices.length > 0 ? Math.min.apply(null, validPrices) : null;

        // Add price answers and highlight lowest value(s)
        priceAnswers.forEach((price, index) => {
          const cell = priceRow.getCell(index + 3); // +3 because first two columns are for section and question
          cell.value = price;
      ...
      category.sections.forEach((section, sectionIndex) => {
        section.questions.forEach((question, questionIndex) => {
          if (section.name === 'Pricing' && question === priceQuestion) {
            return;
          }
    ...
    totalQuotedRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
      if (colNumber > 1) { // Skip the first (empty) cell
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'ffffffff' } }; // Green fill
        cell.font = { bold: true };
        cell.border = borderStyle;
      }
    });
